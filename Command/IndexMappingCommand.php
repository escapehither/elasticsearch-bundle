 ...
 */
class IndexMappingCommand extends ContainerAwareCommand
{
    protected $kernel;
    protected $indexes;
    ...
    {
        $this
            ->setName('escapehither:esm:index:mapping')
            ->setDescription('Get The mapping of an Elastic search index')
            ->setHelp('This command allows you to get the mapping of an index ...')
        ;
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $output->writeln([
            'Get an Elasticsearch index mapping',
            '============',
            '',
        ]);
        ...
        try {
            $output->writeln('The Index mapping ');
            print_r(json_decode(json_encode($index->getMapping())));
        } catch (\Exception $e) {
